...
(function () {
  ...
  class Renderer {
    ...
    loadBitmaps(callback) {
      let loaded = 0;
      const files = ['BALL0256.BMP', 'BALL1256.BMP', 'BALL2256.BMP', 'BALL3256.BMP', 'BALL4256.BMP', 'BALL5256.BMP', 'BALL6256.BMP', 'BALLM256.BMP', 'PLAY256.BMP'];
      ...
      files.forEach(file => {
        ...
        if (EMBEDDED_BALL_BMPS[file]) {
          img.src = EMBEDDED_BALL_BMPS[file];
        } else {
          // PLAY256.BMP playfield background is not embedded — load from assets/
          img.src = 'assets/' + file;
        }
      });
    }
  ...
  class SoundEngine {
    ...
    playMidiTrack(trackName) {
      ...
      const nameUpper = `assets/ODD${trackName.toUpperCase()}.mp3`;
      const nameLower = `assets/odd${trackName.toLowerCase()}.mp3`;
